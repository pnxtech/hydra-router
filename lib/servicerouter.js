...
class ServiceRouter {
  ...
  */
  routeWSMessage(ws, message) {
    ...
    if (msg.to.indexOf('[') > -1 && msg.to.indexOf(']') > -1) {
      ...
      switch (msg.type) {
        ...
        case 'reconnect': {
          if (!msg.body.id) {
            ...
            if (this.config.debugLogging) {
              this.log(INFO, `HR: WS reconnecting to ${msg.body.id}`);
            }
        ...
        case 'wsdir.loc': // locate a client on directory
          {
        ...
        default:
          {
            let newMsg = UMFMessage.createMessage({
              ...
              rmid: msg.mid,
              body: {
                error: 'Unrecognized Hydra Router request'
              }
            });
            this._sendWSMessage(ws, newMsg.toJSON());
          }
      ...
      if (toRoute.serviceName === this.serviceName && msg.forward) {
  ...
  */
  wsDisconnect(ws) {
    if (this.config.debugLogging) {
      this.log(INFO, `HR: WS close connection ${ws.id}`);
    }
    delete this.wsLocalClients[ws.id];
    delete this.wsClients[this.serviceIntanceID][ws.id];
